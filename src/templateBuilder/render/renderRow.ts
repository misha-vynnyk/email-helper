 ...
 * Зовнішня обгортка (padding/width/fill/border/cornerRadius/shadow) — та сама, що й у
 * renderSection (canva-plan-v2.md Stage 2 — Row отримав ті самі опційні поля стилю), і не
 * залежить від кількості колонок. Внутрішня розмітка галузиться лише за columns.length: 1
 * колонка не потребує inline-block-техніки (нема з чим стояти поруч) і рендериться як прямий
 * вміст таблиці — так само, як renderSection кладе childrenHtml прямо в <table>. 2+ колонок
 * лишають класичну font-size:0 + display:inline-block + min-width техніку.
 *
...
export function renderRow(row: RowBlock, nodes: Record<string, BuilderNode>, shell: ShellConfig, availableWidthPx: number): string {
  const c = computeBoxStyle(row, availableWidthPx);
...
<tr>
  <td align="center" style="${paddingStyle}">
    <table align="center" border="0" cellspacing="0" cellpadding="0"${widthAttr}${responsiveClassAttr(undefined, row.responsiveClassNames)}${bgcolorAttr} style="width: 100%;${maxWidthStyle} padding: 0; margin: 0;${extraStyle}" role="presentation">
${innerHtml}
    </table>
  </td>
</tr>
