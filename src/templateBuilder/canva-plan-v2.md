...
### Реалізовано (2026-09-09) — з двома свідомими скороченнями плану, підтвердженими з користувачем

Перед реалізацією прочитано реальний код `render*()`-функцій — виявилось, що дві частини оригінального дизайну зайві:

1. **Пункт "B. Прокидання `availableWidthPx` крізь canvas-дерево" повністю прибрано.** Жодна з `renderText`/`renderImage`/`renderButton`/`renderDivider`/`renderSpacer`/`renderReadyMade` не приймає `availableWidthPx` — усі розміри (button `width`, image `widthPx`) закладені прямо в блок, стилі повністю inline. Лише `renderSection`/`renderRow` (контейнери) споживають `availableWidthPx`, і лише для власного боксу, не для дітей. Жодної нової width-інфраструктури в `CanvasNode`/`NodeDropZone`/`CanvasSectionBox`/`CanvasRowBox` не знадобилось.
2. **Пункт "A. Спільна CSS-інфраструктура" скорочено до одного Google-Fonts-sync-хука**, без `buildStyleBlockCss()`-екстракції з `renderShell.ts`. Її `<style>`-блок — майже виключно `@media`-правила для responsive-класів, що не мають сенсу на canvas (лише десктопна ширина, жодного mobile-simulation режиму нема). Google Fonts — єдина реальна залежність (шрифт не завантажиться без живого `<link>`), закрита через новий `hooks/useSyncGoogleFontsIntoDocument.ts` (один `<link>`, синхронізований з `shell.googleFontsHref`).

**Реалізовано за спрощеним дизайном:** `CanvasChipShell.tsx` — `label: string` → `children: ReactNode`, хром (grip/icon/remove) переїхав у тонку INSIDE-flow смужку зверху (не floating `-top-6`, як у `CanvasWysiwygShell` — та схема потребувала обережної headroom-математики в Stage 1 саме через floating, для багатьох дрібних leaf-чіпів це зайвий клас багів). Шість нових `CanvasXPreview.tsx` (`Text`/`Image`/`Button`/`Divider`/`Spacer`/`ReadyMade`) — кожен викликає відповідний `render*()` буквально з `paddingBottomPx = 0` (canvas-гап уже візуалізується контейнером окремо — Stage 2's `gapPx`/`space-y-1.5`, дублювання дало б подвійний відступ) і вставляє через `dangerouslySetInnerHTML` у `pointer-events: none`-таблицю. `CanvasNode.tsx` — явний switch на кожен тип, дзеркальний до `renderNode.ts`. `CanvasLeafChip.tsx`/`CanvasReadyMadeChip.tsx`/`leafPreviewText()` видалені як мертвий код.

**Тести:** 281→310 (29 нових) — по одному RTL-тесту на кожен `CanvasXPreview` (порівнює `.innerHTML` після `dangerouslySetInnerHTML`-round-trip із відповідним `render*()`-виводом, round-trip з обох боків прибирає крихкість до serialization-квірків), `CanvasChipShell.test.tsx`, `CanvasNode.test.tsx`, `useSyncGoogleFontsIntoDocument.test.tsx`. Новий спільний `testSupport/domHtml.ts`'s `tbodyInnerHtml()` — уникає дублювання цієї round-trip-логіки в 6 файлах.

**Верифіковано headless Chrome (`playwright-core` + системний Google Chrome, `NODE_PATH=./node_modules`) проти вже запущеного dev-сервера:** усі 5 leaf-типів + ready-made "Header (simple)" перетягнуті на canvas показують РЕАЛЬНИЙ рендер (текст у реальному шрифті, справжня кнопка з фоном/радіусом, справжнє зображення, лінія-роздільник, порожній spacer, повна ready-made-розмітка з MSO-коментарями) — не іконку+рядок тексту. Клік прямо на вставленому `<a>` всередині Button-картки НЕ навігує (URL незмінний) і коректно виділяє картку (`pointer-events: none` спрацював). Позначення чекбоксу "Lato" в `GoogleFontPicker` одночасно: (1) додало `<link id="template-builder-google-fonts" href="https://fonts.googleapis.com/css2?family=Lato...">` у `document.head`, (2) живий Text-чіп на canvas негайно перемалювався зі `font-family: Lato, Arial, Helvetica, sans-serif` в `getComputedStyle`. 0 console errors. 310/310 тестів, `tsc --noEmit`/`eslint` чисто (eslint-попередження, що лишились, — усі в некраяних цим stage файлах, наявні до Stage 5).
