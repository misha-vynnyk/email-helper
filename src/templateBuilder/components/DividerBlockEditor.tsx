  ...
  return (
    <div className='grid grid-cols-3 gap-2'>
      ...
      <div className='space-y-1'>
        ...
        <Input type='number' value={block.thicknessPx} onChange={(e) => updateNodeFields(block.id, { thicknessPx: Number(e.target.value) || 0 })} />
      </div>
      <div className='space-y-1'>
        <Label className='text-xs text-muted-foreground'>Width (%)</Label>
        <Input type='number' min={1} max={100} value={block.widthPercent} onChange={(e) => updateNodeFields(block.id, { widthPercent: Number(e.target.value) || 0 })} />
      </div>
    </div>
  );
}
