    ...
    <div className='space-y-2'>
      <div className='grid grid-cols-2 gap-2'>
        <div className='space-y-1'>
          ...
          <Input value={block.alt} onChange={(e) => updateNodeFields(block.id, { alt: e.target.value })} />
        </div>
        <div className='space-y-1'>
          <Label className='text-xs text-muted-foreground'>Width (px)</Label>
          <Input type='number' value={block.widthPx} onChange={(e) => updateNodeFields(block.id, { widthPx: Number(e.target.value) || 0 })} />
        </div>
      </div>
