...
import { Build, Code, Download, Email, PersonAdd, Preview, Security, Send, Settings,Speed } from '@mui/icons-material';
import { alpha,Box, Button, Card, CardContent, Chip, Container, Grid, Stack, Typography } from '@mui/material';
...
const LandingPage: React.FC = () => {
  ...
  const samples = [
    ...
    { id: 'order-ecomerce', name: 'E-commerce Receipt', description: 'Order confirmation template' },
    { id: 'reservation-reminder', name: 'Reservation Reminder', description: 'Booking reminder template' },
  ];
            ...
            <Grid item xs={12} md={6}>
              <Stack spacing={3}>
                ...
                <Typography variant="body1" color="text.secondary" paragraph>
                  Build beautiful, responsive email templates faster than ever with our visual editor, then send them
                  directly via Gmail integration - all in one unified interface.
                </Typography>
        ...
        <Grid container spacing={3}>
          {samples.map((sample) => (
            <Grid item xs={12} sm={6} md={4} key={sample.id}>
              <Card sx={{ '&:hover': { boxShadow: 4 }, cursor: 'pointer' }} onClick={() => handleLoadSample(sample.id)}>
                <CardContent>
                  <Typography variant="h6" gutterBottom>
                    {sample.name}
                  </Typography>
      ...
      <Box sx={{ backgroundColor: 'primary.main', color: 'white', py: 8 }}>
        <Container maxWidth="md" sx={{ textAlign: 'center' }}>
          ...
          <Typography variant="h6" paragraph sx={{ mb: 4 }}>
            Join thousands of developers using EmailBuilder.js to create beautiful, responsive email templates with
            integrated sending capabilities.
          </Typography>
