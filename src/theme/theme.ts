...
const BRAND_BLUE = brandColors.blue;
const BRAND_GREEN = brandColors.green;
const BRAND_RED = brandColors.red;
const BRAND_YELLOW = brandColors.yellow;
const BRAND_PURPLE = brandColors.purple;
const BRAND_BROWN = brandColors.brown;
const STANDARD_FONT_FAMILY = typographyTokens.fontFamily.standard;
const MONOSPACE_FONT_FAMILY = typographyTokens.fontFamily.monospace;

export function createAppTheme(mode: ThemeMode = "light"): Theme {
  const paletteColorsForMode = getPaletteColors(mode);
  const semanticColorsForMode = getSemanticColors(mode);

  const BASE_THEME = createTheme({
    palette: {
      mode,
      background: {
        default: paletteColorsForMode.background.default,
        paper: paletteColorsForMode.background.paper,
      },
      text: {
        primary: paletteColorsForMode.text.primary,
        secondary: paletteColorsForMode.text.secondary,
      },
    },
    typography: {
  ...
  return createTheme(BASE_THEME, {
    palette: {
      ...
      success: semanticColorsForMode.success,
      error: semanticColorsForMode.error,
      cadet: paletteColorsForMode.cadet,
      highlight: {
    ...
    components: {
      ...
      MuiAlert: {
        styleOverrides: {
          ...
          filledSuccess: {
            backgroundColor: semanticColorsForMode.success.main,
          },
        },
      },
      MuiStepLabel: {
        ...
        styleOverrides: {
          root: {
            ...
            "&:hover:not(.Mui-disabled, .Mui-error):before": {
              borderBottom: `1px solid ${BASE_THEME.palette.grey[500]}`,
            },
            "&:after": {
              borderBottom: `1px solid ${BASE_THEME.palette.text.primary}`,
            },
