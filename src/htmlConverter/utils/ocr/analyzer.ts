...
import { autoDetectRoiFracs,blobToCanvas, blobToCanvasFit, cropCanvasByFrac, estimateTextLikelihood } from "./roi/detect";
  ...
  return {
    ...
    analyzeFromUrl: async ({ url, settings, force = false, signal, onProgress }) => {
      ...
      try {
        ...
        // Merge strategy: sort ROIs top-down
        ...
        for (const roiIdx of roiOrder) {
          ...
          // If the second best result is also good/different, append it too —
          // processor.ts does heavy filtering downstream so we can be permissive here.
