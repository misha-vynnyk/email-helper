...
describe("renderRow", () => {
  ...
  it("derives min-width from widthPercent * MASTER_CONTENT_WIDTH_PX (560), not a fixed constant", () => {
  ...
  it("scales min-width per column when widths are uneven", () => {
    ...
    expect(html).toContain("min-width: 224px;"); // 560 * 40%
    expect(html).toContain("min-width: 336px;"); // 560 * 60%
  });

  it("uses the inline-block/font-size:0 stacking attributes on every column cell", () => {
    const html = renderRow(twoColumnRow, "desktop");
    expect(html).toContain('class="sm-w-full"');
  ...
  it("self-wraps in a neutral row with no padding of its own", () => {
    const html = renderRow(twoColumnRow, "desktop");
    expect(html.startsWith('<tr><td align="center">')).toBe(true);
  });
