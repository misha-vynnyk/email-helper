...
export default function ResizablePreview({ children, width, onWidthChange, zoom, minWidth = 280, maxWidth = 1600 }: ResizablePreviewProps) {
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      const newWidth = Math.round(Math.max(minWidth, Math.min(maxWidth, startWidthRef.current + widthChange)));
  ...
  return (
    <div className='relative w-full py-4'>
      ...
      {!isResponsive && (
        <div className='flex justify-center mb-2'>
          <div
            ...
            <span className='text-xs font-bold text-primary font-mono'>{displayWidth}px</span>
            {zoom !== 1 && <span className='text-[10px] text-muted-foreground font-mono'>(zoom {Math.round(zoom * 100)}%)</span>}
          </div>
        </div>
      )}
      ...
      <div className='flex justify-center items-start relative min-h-[200px]'>
        ...
        {!isResponsive && (
          <div
            ...
            <div className={`w-1.5 rounded-full transition-all duration-200 flex items-center justify-center ${isDragging ? "bg-primary h-20" : "bg-primary/50 h-14 group-hover:bg-primary group-hover:h-20 group-hover:shadow-[0_0_12px_rgba(var(--primary),0.5)]"}`}>
        ...
        <div
          ...
          {!isResponsive && <div className='absolute -top-[2px] -left-[2px] -right-[2px] -bottom-[2px] border-2 border-primary/30 rounded pointer-events-none z-[1]' />}
          {children}
        </div>
      ...
      {isDragging && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className='fixed top-5 left-1/2 -translate-x-1/2 bg-primary text-primary-foreground px-6 py-3 rounded-lg text-lg font-bold font-mono pointer-events-none z-[10000] shadow-lg flex items-center gap-3'>
          <span>{currentWidth}px</span>
          {zoom !== 1 && <span className='text-xs opacity-80 font-normal'>(zoom {Math.round(zoom * 100)}%)</span>}
        </motion.div>
      )}
      ...
      {isDragging && (
        <div className='fixed bottom-5 left-1/2 -translate-x-1/2 flex gap-2 z-[10000]'>
          {[
            ...
            return (
              <div key={bp.width} className={`px-3 py-1.5 rounded-md text-xs shadow-md transition-all duration-150 ${isActive ? "bg-primary text-primary-foreground font-semibold border-none" : "bg-background/95 text-muted-foreground font-normal border border-border"}`}>
                {bp.label} ({bp.width})
              </div>
            );
          })}
