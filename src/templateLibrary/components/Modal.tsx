 ...
 */
export default function Modal({ open, onClose, title, headerExtra, children, actionsRow, maxWidthClass = "max-w-2xl" }: ModalProps) {
  ...
  return createPortal(
    <div className='fixed inset-0 z-[150] flex items-center justify-center p-4 sm:p-6' role='dialog' aria-modal='true'>
      {/* Backdrop overlay */}
      <div className='absolute inset-0 bg-background/80 backdrop-blur-sm transition-opacity' onClick={onClose} aria-hidden='true' />
      ...
      <div className={`relative w-full ${maxWidthClass} bg-card border border-border/50 rounded-2xl shadow-soft flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200 max-h-[95vh]`}>
        ...
        <div className='flex items-center justify-between px-6 py-4 border-b border-border/50 min-h-[72px]'>
          ...
          <button onClick={onClose} className='p-2 text-muted-foreground hover:bg-muted hover:text-foreground rounded-full transition-colors flex-shrink-0' aria-label='Close dialog'>
            <CloseIcon size={20} />
          </button>
        </div>
        ...
        {actionsRow && <div className='flex items-center justify-end gap-3 px-6 py-4 border-t border-border/50 bg-muted/10'>{actionsRow}</div>}
      </div>
    </div>,
    document.body
  );
