 ...
 */
export async function convertImagesServerBatch(files: File[], settings: ConversionSettings): Promise<Blob[]> {
  const formData = new FormData();
  ...
  const response = await fetch(`${API_URL}/api/image/convert-batch`, {
    method: 'POST',
    body: formData,
  });
