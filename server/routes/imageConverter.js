...
router.post("/convert-from-url", async (req, res) => {
  try {
    ...
    const attempts = [
      {
        name: "browser-like",
        headers: {
          // Some CDNs block unknown UA / require Accept+Referer
          "User-Agent":
      ...
      {
        name: "minimal",
        headers: { "User-Agent": "Email-Helper/1.0", Accept: "image/*,*/*;q=0.8" },
      },
    ];
    ...
    if (!inputBuffer) {
      console.warn(`convert-from-url: fetch failed (${lastStatus}) ${url}`);
    ...
    if (isInputGif && format === "gif") {
      ...
      res.set("Content-Type", "image/gif");
      res.set("X-Original-Size", String(inputBuffer.length));
 ...
 */
router.post("/convert-batch", upload.array("images", 20), async (req, res) => {
  try {
